import pygame
from LibreriaGeneral import *
pygame.init()
#----------------------------------------------------
width = 1280
high =920
window = pygame.display.set_mode([width,high])
middle = [width/2,high/2]
end = False

#PRIMERA CARA

a1 = [0,0]
a2 = PolarToCartesian(50,30)
a3 = Traslacion(a2,0,50)
a4 = PolarToCartesian(50,210)

a1s = CartToScreen(middle,a1)
a2s = CartToScreen(a1s,a2)
a3s = CartToScreen(a1s,a3)
a4s = CartToScreen(a3s,a4)

PrimerCara = [a1s,a2s,a3s,a4s]

##SEGUNDA CARA
b1 = PolarToCartesian(50,150)
b2 = PolarToCartesian(50,30)

b1s = CartToScreen(a3s,b1)
b2s = CartToScreen(b1s,b2)


SegundaCara = [a3s,b1s,b2s]

#TERCER POLIGONO
c1 = PolarToCartesian(50,330)
c2 = Traslacion(c1,0,-50)

c1s = CartToScreen(b2s,c1)
c2s = CartToScreen(b2s,c2)

TercerCara = [a3s,b2s,c1s,c2s]

#CUARTO POLIGONO
d1 = PolarToCartesian(50,30)
d2 = Traslacion(d1,0,50)

d1s = CartToScreen(c2s,d1)
d2s = CartToScreen(c2s,d2)

CuartaCara = [c2s,d1s,d2s,c1s]

#quinta cara
e1 = PolarToCartesian(150,150)
e2 = Traslacion(e1,0,125)
e3 = PolarToCartesian(50,330)
e4 = Traslacion(e3,0,-75)

e1s = CartToScreen(a1s,e1)
e2s = CartToScreen(a1s,e2)
e3s = CartToScreen(e2s,e3)
e4s = CartToScreen(e2s,e4)


QuintaCara = [a1s,e1s,e2s,e3s,e4s,a4s]

#sexta cara
f1 = PolarToCartesian(150,30)

f1s = CartToScreen(e4s,f1)



SextaCara = [d2s,c1s,b2s,b1s,a3s,a4s,e4s,f1s]

#septima cara
g1 = Traslacion(f1,0,75)

g1s = CartToScreen(e4s,g1)

SeptimaCara = [f1s,g1s,e3s,e4s]

#octava cara

h1 = PolarToCartesian(50,150)


h1s = CartToScreen(g1s,h1)


OctavaCara = [e2s,e3s,g1s,h1s]

"""

#NOVENA CARA

NovenaCara = [a5s,e3s,h1s,g1s,f1s,a4s]

#decima cara

DecimaCara = [e1s,e2s,d2s]
"""
if __name__ == "__main__":
    #drawPlane(window,middle)
    while not end:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                end = True

        drawPolygon(window,SelectColor('Red'),PrimerCara)
        drawPolygon(window,SelectColor('Red'),SegundaCara)
        drawPolygon(window,SelectColor('Red'),TercerCara)
        drawPolygon(window,SelectColor('Red'),CuartaCara)
        drawPolygon(window,SelectColor('Red'),QuintaCara)
        drawPolygon(window,SelectColor('Red'),SextaCara)
        drawPolygon(window,SelectColor('Red'),SeptimaCara)
        drawPolygon(window,SelectColor('Red'),OctavaCara)
        """drawPolygon(window,SelectColor('Red'),NovenaCara)
        drawPolygon(window,SelectColor('Red'),DecimaCara)"""
